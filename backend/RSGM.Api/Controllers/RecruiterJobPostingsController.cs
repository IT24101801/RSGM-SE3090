        ...
        (RecruiterJobResult Result, RecruiterJobPostingResponse? Job) result)
    {
        return result.Result switch
        {
            ...
            RecruiterJobResult.InvalidEmploymentType => BadRequest(new
                { message = "Employment type must be FullTime, PartTime, Contract, or Internship." }),
            RecruiterJobResult.InvalidWorkMode => BadRequest(new
                { message = "Work mode must be OnSite, Remote, or Hybrid." }),
            RecruiterJobResult.InvalidExperienceLevel => BadRequest(new
                { message = "Experience level must be Entry, Junior, Mid, or Senior." }),
            RecruiterJobResult.InvalidExperience => BadRequest(new
                { message = "Minimum experience is required unless this is an internship." }),
            RecruiterJobResult.InvalidSalary => BadRequest(new
                { message = "Check the salary range and provide a three-letter currency when salary is entered." }),
