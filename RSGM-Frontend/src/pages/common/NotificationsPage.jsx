...
export default function NotificationsPage({ role }) {
  ...
  const accent = role === "panelist" ? "amber" : role === "hr" ? "emerald" : role === "recruiter" ? "blue" : "violet";
  ...
  const color = accent === "amber" ? "bg-amber-100 text-amber-700" : accent === "emerald" ? "bg-emerald-100 text-emerald-700" : accent === "blue" ? "bg-blue-100 text-blue-700" : "bg-violet-100 text-violet-700";
  ...
  return <div>
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-semibold ${color}`}><Bell size={13} />YOUR UPDATES</p>
        ...
        <p className="mt-2 text-neutral-500">Interview, recommendation, salary and offer workflow updates.</p></div>
    ...
    <div className="mt-8 space-y-3">
      ...
      {items.map((item) => <button type="button" key={item.id} onClick={() => open(item)} disabled={busy} aria-label={`${item.title}. ${item.readAt ? "Read" : "Mark as read"}`} className={`w-full rounded-2xl border bg-white p-5 text-left shadow-sm transition enabled:hover:shadow-md ${item.readAt ? "border-neutral-200" : accent === "amber" ? "border-amber-300" : accent === "emerald" ? "border-emerald-300" : accent === "blue" ? "border-blue-300" : "border-violet-300"}`}>
        <div className="flex items-start gap-3">
          <span className={`rounded-xl p-2.5 ${color}`}><Bell size={16} /></span>
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2"><span className="font-semibold text-neutral-900">{item.title}</span>{!item.readAt && <span className={`h-2 w-2 rounded-full ${accent === "amber" ? "bg-amber-500" : accent === "emerald" ? "bg-emerald-500" : accent === "blue" ? "bg-blue-500" : "bg-violet-500"}`} />}</span>
