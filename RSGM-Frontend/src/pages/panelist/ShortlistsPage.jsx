...
export default function ShortlistsPage() {
  ...
  return <div><p className="text-xs font-semibold uppercase text-amber-600">Hiring panelist</p><h1 className="mt-3 text-3xl font-semibold">Ranked shortlists</h1>
    <p className="mt-2 text-sm text-neutral-500">Select a candidate and propose a time shared by you, the recruiter, and HR. Candidates confirm the proposal before the interview.</p>
    <Link className="mt-3 inline-block text-sm font-semibold text-amber-700" to="/panelist/availability">Manage your availability →</Link>
    ...
    {selection && <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/50 p-4"><form onSubmit={submit} className="w-full max-w-lg space-y-4 rounded-2xl bg-white p-6 shadow-2xl"><div className="flex justify-between"><h2 className="text-lg font-semibold">Interview · {selection.candidate.candidate}</h2><button type="button" onClick={() => setSelection(null)} aria-label="Close">✕</button></div>
      ...
      <label className="block text-sm font-medium">Shared one-hour slot<select required className="mt-2 w-full rounded-xl border border-neutral-200 p-3" value={start} onChange={(e) => setStart(e.target.value)}><option value="">Select a slot</option>{slots.map((iso) => <option key={iso} value={iso}>{new Date(iso).toLocaleString()}</option>)}</select></label>
      {!slots.length && <p className="text-sm text-amber-700">No shared slots yet. Ask the recruiter and HR Manager to add matching availability.</p>}
