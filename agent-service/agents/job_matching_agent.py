...
class JobMatchingAgent:
    ...
    def run(self, candidate: CandidateProfileInput, jobs: list[JobInput],
            profile: ProfileAnalysis) -> list[JobMatch]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are the RSGM Job Matching Agent.
...
Do not claim unrecorded skills or experience. Return structured output only."""),
        ...
        chain = prompt | get_llm().with_structured_output(MatchExplanationOutput)
        explanations = chain.invoke({
            "profile": profile.model_dump_json(by_alias=True),
            "matches": "\n".join(item.model_dump_json(by_alias=True) for item in ranked),
        })
        lookup = {item.job_id: item.explanation for item in explanations.explanations}
        for match in ranked:
            match.explanation = lookup.get(str(match.job_id),
